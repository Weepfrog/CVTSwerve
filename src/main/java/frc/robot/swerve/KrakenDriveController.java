...
public class KrakenDriveController implements DriveController {
    public static final double WHEEL_DIAMETER = Units.inchesToMeters(4.0);
    // This is the L2 gearing
    public static final double DRIVE_REDUCTION = (14.0 / 50.0) * (27.0 / 17.0) * (15.0 / 45.0);

    public static final double MAX_VELOCITY_METERS_PER_SECOND = 
            6000.0 / 60.0 * DRIVE_REDUCTION * (Math.PI * WHEEL_DIAMETER);
    
    // Note: Phoenix 6 library uses rotations, so no need for tick count
    private static final double KRAKEN_DISTANCE_PER_UNIT = Math.PI * WHEEL_DIAMETER * DRIVE_REDUCTION;
    ...
    public KrakenDriveController(int id) {
    ...
    public double getStateVelocity() {
        return m_motor.getVelocity().getValueAsDouble() * KRAKEN_DISTANCE_PER_UNIT;
    }

    @Override
    public double getWheelDistance() {
        return m_motor.getPosition().getValueAsDouble() * KRAKEN_DISTANCE_PER_UNIT;
    }
